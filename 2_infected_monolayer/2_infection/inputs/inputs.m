classdef inputs
   properties (Constant)
       ...
       general_run_cluster                  = false      ;     % run code on cluster true/false
       ...
       monolayer_infection_active           = true      ;
       ...
       monolayer_infection_propagation      = true      ;     % Propagate infection
       monolayer_infection_step_propagation = 90        ;     % Apply propagation after this number of steps (reach an equilibrium first)
       monolayer_infection_propg_constant   = 20        ;     % [µm^2/min] 1.25 before

       monolayer_extrusion_active           = true      ;
       monolayer_extrusion_min_inf_area     = 6500      ;     % Minimum infected area to start the extrusion [µm2]
       step_to_get_reference_area           = 10        ;     % Initial state where we start computing the change of area, so we get the reference area in that step
       extrude_every_X_steps                = 3         ;     % Extrude one cell every X steps
       ...
       forces_contraction_limit             = 4000      ;     % [pN] Only for contraction mode = 3
       force_contraction_scale              = 0.2       ;     % Scale factor, only for contraction mode = 3
       force_contraction_infection          = 1750      ;     % [pN] Only for contraction mode = 3
       ...
       forces_crawling_infection_active     = true      ;
       ...
       % _________________ (6) Cell extrusion forces ______________________
       forces_extrusion_magnitude           = 800       ;     % [pN]
