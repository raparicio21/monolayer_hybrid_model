function vtkwrite_fem_stress( filename,dataType,varargin )
...
% 2. Title
fprintf(fid, 'Finite Element Results\n');
...
% This is 10 times faster and it is okay with Paraview format
        ...
        % Write connectivity
        % Count all vertices for all polygons
...
%             %fprintf(fid, '%d %d \n', varargin{4}(j), varargin{5}{j,:});
%             fprintf(fid, '%d ', varargin{4}(j));
%             fprintf(fid, '%d ', varargin{5}{j,:}-1);
%             fprintf(fid,'\n');
%         end

        fprintf(fid, sprintf('\nCELL_TYPES %d\n',size(varargin{4},1)));
        % cell_type depends on your type of element, see figure 2 here:
        % https://kitware.github.io/vtk-examples/site/VTKFileFormats/
        if num_vertices_per_element == 6
            cell_types_number = 22 * ones(size(varargin{4},1),1)';
        elseif num_vertices_per_element == 3
            cell_types_number =  5 * ones(size(varargin{4},1),1)';
        end        
        fprintf(fid, '%d ', cell_types_number);
