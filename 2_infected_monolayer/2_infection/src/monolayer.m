classdef monolayer
    ...
    properties
        ...
        force_extruding_cell              % List of cell forces for extruding forces (centroid) only for infected cells
        ...
        infection_age                     % Infection age (steps one cell has been infected) 
        chosen_cell_to_extrude            % Index of the chosen cell to extrude in the following step i+1
        extrusion_counter                 % Counter that goes from 0 to 2 steps to apply extruding forces
        exterior_infected_cells           % List of INfected cells at the border of the infection
        first_extrusion                   % Was there any first extrusion? true or false
        first_evaluation_extrusion        % True: you have already evaluated extrusion
        ...
        total_infection_area              % Sum of all polygons that are infected
    ...
    methods
        ...
        function obj = generateGeometry(obj)
            ...
            if inputs.monolayer_extrusion_active == true
                ...
                % Get infection area
                infection_area = obj.total_infection_area;

                % First extrusion == false at the beginning
                % When the infection area > min inf area for extrusion,
                % then extrusion is allowed. However, the area of inf might
                % be reduced again, that's the reason we need to set
                % first_extrusion == true to get into the loop later.

                if infection_area >= inputs.monolayer_extrusion_min_inf_area || obj.first_evaluation_extrusion == true
                    ...
                    if any(list_inf_cells)
                        ...
                        % (1) Get infection age
                        infect_age    = obj.infection_age ;

                        % (2) Avoid extrusion of infected cells at
                        % the border, get a list of infected interior
                        % cells from previous step
                        combined_list = list_inf_cells - obj.exterior_infected_cells';

                        % (3) Avoid cell indices that have been already
                        % extruded
                        extrusion_list = obj.cell_extrusion                ;
                        combined_list  = combined_list - extrusion_list    ;

                        % Get indices of possible extruding cells
                        extrusion_candidates = find(combined_list == 1)    ;
                        candidates_length    = length(extrusion_candidates);

                        if any(candidates_length)

                            
                            
                            % (A) Compute a score between [0,1], [min,max]
                            % associated to infection age
                            numb_cells    = candidates_length               ;
                            values        = infect_age(extrusion_candidates);

                            min_value     = min(values)                     ;
                            max_value     = max(values)                     ;

                            score_inf_age = zeros(numb_cells,1)             ;
                            ...
                            if min_value == max_value
                                ...
                                for i = 1:length(values)
                                    % Compute scores between 0 and 1 as a lineal function between the min and max value
                                    score_inf_age(i) = (values(i) - min_value) / (max_value - min_value);
                            ...
                            % (B) Compute a score between [0,1], [min,max]
                            % associated to area change
                            area_chang            = obj.area_change                 ;
                            values                = area_chang(extrusion_candidates);
                            ...
                            score_inf_area_change = zeros(numb_cells,1)             ;

                            if min_value == max_value
                                score_inf_area_change(:) = 1;
                            else
                                for i = 1:length(values)
                                    % Compute scores between 0 and 1 as a lineal function between the min and max value
                                    score_inf_area_change(i) = (values(i) - min_value) / (max_value - min_value);
                                   
                                end
                            end


                            total_score = score_inf_age + score_inf_area_change;                

                            [max_value,id] = max(total_score);

                            
                            % First extrusion
                            if obj.first_extrusion == false

                                if obj.extrusion_counter == -1

                                    % Chosen cell to extrude
                                    index = extrusion_candidates(id);
                                    obj.chosen_cell_to_extrude     = index ;
                                    obj.extrusion_counter          = 0     ;
                                    obj.first_evaluation_extrusion = true  ;

                                elseif obj.extrusion_counter == 3

                                    % Index saved from previous step
                                    index     = obj.chosen_cell_to_extrude;
                                    % Clean structure
                                    obj.chosen_cell_to_extrude     = []    ;

                                    % Here it's when you extrude the cell
                                    obj.cell_extrusion(index)      = 1     ; 
                                    obj.first_extrusion            = true  ;                                    
                                    disp('Cell extruded')
                                    obj.extrusion_counter          = -1    ;
                                ...
                                % Extrusions after first extrusion
            ...
            if inputs.monolayer_infection_propagation == true && obj.step >= inputs.monolayer_infection_step_propagation
                ...
                % Compute and update infection age
                obj.infection_age    = obj.infection_age + list_inf_cells                    ;
        ...
        function obj = calculateForces(obj,FEM_results)
            ...
            obj.force_extruding_cell       = zeros(size(obj.monolayer_cell_points,1),3);
            ...
            mon_inf_cells_border = obj.exterior_infected_cells;
            ...
            if any(obj.chosen_cell_to_extrude)
                next_extruding_cell  = obj.chosen_cell_to_extrude ;
            else
                next_extruding_cell  = -1                         ;
            end
            ...
            for i = 1:number_cells
                ...
                if extruding_cells(i) == 0    % Non-extruding cell  
                    ...
                    % Avoid next extruding cell itself + its interaction with its neighbours
                    if  i ~= next_extruding_cell
                        neighbours_id              = list_neighbours{i}' ;
                        vector_f_interaction(i,:)  = my_forces.calculateInteraction(i,neighbours_id,mon_cell_radius,mon_cell_pos,mon_cell_infection,next_extruding_cell); 
                    end
            ...
            if inputs.forces_contraction_active == true
                ...
                if inputs.forces_contraction_mode == 1
                    ...
                    for i = 1:number_cells  
                        if extruding_cells(i) == 0    % Non-extruding cell 
                            [vector_f_contraction(i,:),vert_forces, cont_vert_coor, cont_vert_forc] = my_forces.calculateContraction(i, mon_cell_infection, mon_inf_cells_border, mon_step, mon_cell_pos, ...
                                    mon_polygons, mon_regions, mon_vertices, vert_forces, cont_vert_coor, cont_vert_forc, obj.first_extrusion);
                        end
                    end
...
%                    % Considering averaged cell force_II  from the resultant stress tensor (principal stress  II)
    ...
    %                                 mon_vertices, vert_forces_2, prot_vert_coor, prot_vert_forc);
            ...
            if inputs.forces_crawling_infection_active == true
                ...
                % Wait some steps for the reaction of the cells (when there
                % is propagation)
                if inputs.monolayer_infection_propagation == true

                    % (1) Border of infection
                    list_surr_cells    = obj.surrounder_cell            ;
                    surr_indices       = find(list_surr_cells == 1)     ;
                    surr_length        = length(surr_indices)           ;
    
                    if surr_length ~= 0
                        % Retrieve infection domain
                        border_polygon     = obj.infected_polygon           ;
                        border_vertices    = border_polygon.Vertices        ;
                        infection_area     = obj.total_infection_area       ;


                        %.................. 1st line of neigh. uninf. cells .................

                        % Idea: 
                        % (1) When there is no first extrusion, we apply some infection forces on the uninfected cells
                        % that are the 1st line of neigbouring cells to the infection focus. However, we apply reduced
                        % forces. 
                        % (2) Once that the first extrusion happens we will apply non-reduced forces. 
                        ...
                        for i = 1 : surr_length
                            ...
                            obj = my_forces.calculateInfectionCrawling(cell_number,obj,mon_regions,border_vertices,factor);
                        end
                    



                        %.................. 2nd and 3rd line of neigh. uninf. cells .................
                        % Idea:
                        % We only apply infection forces on the 2nd and 3rd line of neighbouring uninfected cells after the 1st
                        % extrusion happens
    
                        if obj.first_extrusion == true
            
                            % (2) 2nd line of cells
                            list_surr_cells    = obj.surrounder_cell_2nd        ;
                            surr_indices       = find(list_surr_cells == 1)     ;
                            surr_length        = length(surr_indices)           ;

                            if surr_length ~= 0
                                % Retrieve infection domain
                                border_polygon     = obj.infected_polygon_1st       ;
                                border_vertices    = border_polygon.Vertices        ;
                                factor             = 1                              ;
                            ...
                            % (3) 3rd line of cells
                            list_surr_cells    = obj.surrounder_cell_3rd        ;
                            surr_indices       = find(list_surr_cells == 1)     ;
                            surr_length        = length(surr_indices)           ;

                            if surr_length ~= 0
                                % Retrieve infection domain
                                border_polygon     = obj.infected_polygon_2nd       ;
                                border_vertices    = border_polygon.Vertices        ;
                                factor             = 1                              ;
                    ...
                    % In the case of no propagation, we do not wait to
                    % apply the infection forces
                    ...
                    if surr_length ~= 0
                        ...
                        for i = 1 : surr_length
                            ...
                            cell_number = surr_indices(i);
                            % Compute infection forces
                            obj = my_forces.calculateInfectionCrawling(cell_number,obj,mon_regions,border_vertices);
                        end
                    end
            ...
            if inputs.monolayer_extrusion_active == true 
                ...
                if inputs.monolayer_infection_active == true 
                    ...
                    if any(obj.chosen_cell_to_extrude) || obj.extrusion_counter == 0 || obj.extrusion_counter == 1 || obj.extrusion_counter == 2 

                        % % (1) APPLY EXTRUSION FORCES TO ALL CELLS
                        % 
                        %  % Find infected cells
                        % list_inf_cells     = obj.cell_infection             ;
                        % inf_indices        = find(list_inf_cells == 1)      ;
                        % inf_length         = length(inf_indices)            ;
                        % 
                        % if inf_length ~= 0
                        %     % Retrieve extruding cell information
                        %     extr_cell_index    = obj.chosen_cell_to_extrude                 ;
                        %     extruding_cell_pos = obj.monolayer_cell_points(extr_cell_index,1:2);
                        % 
                        %     % Remove extruding cell from inf_indices
                        %     contains_index = any( inf_indices == extr_cell_index);
                        %     if contains_index == true
                        % 
                        %         cell_index = inf_indices == extr_cell_index;
                        %         inf_indices(cell_index) = [];
                        % 
                        %     end
                        % 
                        %     % Compute extrusion forces            
                        %     for i = 1 : length(inf_indices)
                        % 
                        %         iterator     = inf_indices(i);
                        %         inf_cell_pos = obj.cell_position(iterator,1:2);
                        % 
                        %         obj          = my_forces.calculateExtrusionForces(obj,inf_cell_pos,iterator,extruding_cell_pos);
                        %     end
                        % end
                        % 


                        % (2) or APPLY EXTRUSION FORCES TO THE NEIGHBOURS
                        % OF THE NEXT EXTRUDING CELL
                        ...
                        % Get its neighbours
                        neighbours_id      = list_neighbours{extr_cell_index}';

                        % Compute extrusion forces   
                        for i = 1 : length(neighbours_id)

                            iterator       = neighbours_id(i);
                            neigh_cell_pos = obj.cell_position(iterator,1:2);
                            obj            = my_forces.calculateExtrusionForces(obj,neigh_cell_pos,iterator,extruding_cell_pos);
                        ...
                        obj.extrusion_counter = obj.extrusion_counter + 1;
...
%             hold off
        ...
        function [monolayer,fem_results] = computeAverageStress(monolayer,fem_results,mesh_object)
            ...
            if step_i > inputs.step_to_get_reference_area
            ...
            for i = 1:numb_cells
                ...
                if list_extruding_cells(i) == 0 % Non-extruding cell
                    ...
                    % Compute area change from step 10 for example
                    step_i           =  monolayer.step    ;
                    if step_i == inputs.step_to_get_reference_area
                        cell_reference_area(i) = sum_areas_cell;

                    elseif step_i > inputs.step_to_get_reference_area
