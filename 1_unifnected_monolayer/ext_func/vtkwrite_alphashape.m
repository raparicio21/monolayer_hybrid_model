function vtkwrite_alphashape( filename,dataType,varargin )
...
switch upper(dataType)       
    case {'STRUCTURED_GRID','ALPHASHAPE'}
...
%         end
        ...
        % Write voronoi regions
        fprintf(fid, sprintf('\nPOLYGONS %d %d\n',1 , 1 + n_elements)); 
        % number of polygons first and then number of polygons + number
        % elements you write on the list

        % Manually, assuming there is only 1 alpha shape and the order is 1
        % 2 3 4....
        for j = 1:length(1)
            %fprintf(fid, '%d %d \n', varargin{4}(j), varargin{5}{j,:});
            fprintf(fid, '%d ', n_elements);
            fprintf(fid, '%d ', 0:1:(n_elements-1));
            fprintf(fid,'\n');
        end
